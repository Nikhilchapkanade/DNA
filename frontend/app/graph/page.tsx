...
// Bio-DNA OS Graph Visualizer Page
...
export default async function GraphPage() {
  ...
  return (
    <div className="grid" style={{ gap: 16 }}>
      <div className="card">
        <h2 className="section-title">Bio-DNA OS Lineage Graph</h2>
        <p className="section-kicker">See the blast radius instantly. Hotter edges and nodes indicate structural drift and incident pressure.</p>
      </div>

      <section className="grid-3">
        <article className="card">
          <h3>Downstream Target Invalidation Scale</h3>
          <p className="metric-value">{graph.metrics.blast_radius * 250} candidates</p>
        </article>
        <article className="card">
          <h3>Structural Drift Velocity</h3>
      ...
      <section className="grid-3">
        <article className="card">
          <h3>Total Biological Assets</h3>
          <p className="metric-value">{graph.nodes.length}</p>
        </article>
        <article className="card">
          <h3>Total Lineage Edges</h3>
        ...
        <article className="card">
          ...
          <span className={`data-pill ${graph.anomalies.length > 0 ? "bad" : "good"}`}>
            {graph.anomalies.length > 0 ? "anomalous path detected" : "all clear"}
          </span>
        </article>
      </section>
      ...
      {hasData ? (
        ...
        <div className="card">
          <p className="section-kicker">No lineage data yet. Run demo magic path or sync openmetadata.</p>
        </div>
      )}

      <div className="card">
        <h3>Bio-Pipeline Anomalies Feed</h3>
