...
export default async function TimelinePage() {
  ...
  if (!timeline) {
    return (
      <div className="card">
        <h3 className="section-title">Bio-DNA OS Timeline Unavailable</h3>
        <p className="section-kicker">No snapshots found. Run the magic demo path to seed the peptide screening storyline.</p>
      </div>
    );
  }
  ...
  return (
    <div className="grid" style={{ gap: 16 }}>
      <div className="card">
        ...
        <p className="section-kicker">Replay the exact failure timeline from mutation trigger to downstream screening impact.</p>
        <div className="question-chips">
          ...
          <span className="data-pill warn">Drift Velocity {timeline.metrics.risk_score.toFixed(0)}</span>
          <span className="data-pill good">Invalidated Targets {timeline.metrics.blast_radius * 250}</span>
        </div>
      </div>
      ...
      <div className="card timeline">
        {timeline.snapshots.map((s, i) => (
          <div key={s.captured_at + i} className="timeline-item">
            ...
            <p style={{ margin: "4px 0 9px" }}>Pipeline Bio-Integrity Index: {s.trust_score.toFixed(0)}</p>
        ...
        <div className="timeline">
          {timeline.timeline_events.map((e, i) => (
            <div key={`${e.dataset}-${i}`} className="timeline-item">
              ...
              <p style={{ marginTop: 6 }}>
                <span className={`data-pill ${e.event_type.includes("break") || e.event_type.includes("drift") || e.event_type.includes("mutation") ? "bad" : "good"}`}>
                  {e.event_type}
                </span>
              </p>
