...
# Bio-DNA OS BrightData MCP Client
...
class BrightDataMCPClient:
    ...
    def _build_url(self) -> str:
        if not self.settings.brightdata_api_token:
            raise ValueError("BRIGHTDATA_API_TOKEN is required for remote validation")
    ...
    async def discover(self, query: str) -> dict:
        ...
        if not self.settings.brightdata_api_token:
            return {
                "tool": "discover",
                "structured": {},
                "text": "Local offline execution: BrightData MCP validation bypassed successfully.",
                "is_error": False,
            }
